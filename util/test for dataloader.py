...
from new_Multi30k import newMulti30k
...
class Dataloader:
    ...
    def make_dataset(self):
        ...
        test_data = newMulti30k(root=os.path.join(os.path.abspath(os.path.dirname(os.getcwd())), 'data'),
    ...
    def make_iter(self, train, validate, test, batch_size, device):
        if self.ext == ('de', 'en'):
            source_text_transform = lambda x: (
                        [self.source_vocab[self.init_token]] + [self.source_vocab[token] for token in self.tokenize_de(x)]
                        + [self.source_vocab[self.eos_token]])
            target_text_transform = lambda x: (
                        [self.target_vocab[self.init_token]] + [self.target_vocab[token] for token in self.tokenize_en(x)]
                        + [self.target_vocab[self.eos_token]])
        elif self.ext == ('en', 'de'):
            source_text_transform = lambda x: (
                        [self.source_vocab[self.init_token]] + [self.source_vocab[token] for token in self.tokenize_en(x)]
                        + [self.source_vocab[self.eos_token]])
            target_text_transform = lambda x: (
                        [self.target_vocab[self.init_token]] + [self.target_vocab[token] for token in self.tokenize_de(x)]
                        + [self.target_vocab[self.eos_token]])
        ...
        def batch_sampler(batch):
            if self.ext == ('de', 'en'):
                indices = [(i, len(self.tokenize_de(x[0]))) for i, x in enumerate(batch)]  # the length of de is enough to sort
            elif self.ext == ('en', 'de'):
                indices = [(i, len(self.tokenize_en(x[0]))) for i, x in enumerate(batch)]
        ...
        train_iterator = DataLoader(list(train), batch_sampler=batch_sampler(list(train)),
                                       collate_fn=collate_batch)
        valid_iterator = DataLoader(list(validate), batch_sampler=batch_sampler(list(validate)),
                                       collate_fn=collate_batch)
        test_iterator = DataLoader(list(test), batch_sampler=batch_sampler(list(test)),
                                       collate_fn=collate_batch)
